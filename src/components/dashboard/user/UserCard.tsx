...
const useUserCard = (refetchUsers: () => void) => {
    const [editingUserIndex, setEditingUserIndex] = React.useState<number | null>(null);
    ...
    const {mutate: updateUserMutation, isPending: updateUserLoading} = trpc.user.update.useMutation();
    const {mutate: deleteUserMutation, isPending: deleteUserLoading} = trpc.user.delete.useMutation();
...
export const UserCard: FC<Props> = ({ users, openUserDialog, refetchUsers }) => {
    const {
        editingUserIndex,
        setEditingUserIndex,
        editingUserNameRef,
                    ...
                    {users?.map((user, userIndex) => (
                        <TableRow key={user.userId}>
                            ...
                            <TableCell>
                                ...
                                <AutoClosePopover userIndex={userIndex} userId={user.userId}/>
                                )}
                            </TableCell>
                        </TableRow>
                    ))}
