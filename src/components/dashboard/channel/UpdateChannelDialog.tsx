...
export const UpdateChannelDialog: React.FC<ChannelDialogProps> = ({
  ...
  const {mutate: updateChannelMutation, isPending: updateChannelLoading} = trpc.channel.update.useMutation();
  ...
  const {
    ...
    resolver: zodResolver(channelSchema.pick({
      channelName: true,
      day: true,
    })),
    defaultValues: {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="w-full max-w-md">
        <DialogTitle className="text-lg font-bold">Update Channel Configs</DialogTitle>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div>
            ...
            />
            {errors.channelName && (
              <p className="text-red-600 text-sm mt-1">{errors.channelName.message}</p>
            )}
          </div>
