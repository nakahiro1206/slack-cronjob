...
export const Test = ({ refetchUpcomingSlots }: TestProps) => {
	...
	const { mutate: notifyMutation, isPending: loadingNotifyMutation } =
		trpc.cronjob.testNotify.useMutation();
	...
	const handleSelectAll = () => {
		if (selectedChannels.size === channels.length) {
			...
			setSelectedChannels(
				new Set(channels.map((channel) => channel.channelId) || []),
			);
		}
	};
		...
		<>
				...
				<div className="flex items-center justify-between mb-4">
					...
					<div className="flex items-center space-x-2">
						<Checkbox
							id="select-all"
							checked={
								selectedChannels.size === channels.length && channels.length > 0
							}
							...
							<div className="flex items-center justify-between mb-3">
								<div className="flex items-center space-x-2">
									<Checkbox
										...
										onCheckedChange={() =>
											handleChannelToggle(channel.channelId)
										}
									/>
			...
			<div className="mb-8">
				...
				<p className="mb-4 text-gray-600">
					Select the channels you want to notify, then click the button below to
					manually trigger the notify. It will send messages to all selected
					channels.
				</p>
			...
			<Card className="bg-yellow-50 p-6 rounded-lg border border-yellow-200">
				...
				<div className="text-yellow-700 space-y-2">
					<p>
						• Select one or more channels using the checkboxes above, then click
						the test button to send messages to those channels.
					</p>
					<p>
						• Channel members are mentioned in the Slack message using their
						user IDs.
					</p>
				</div>
			</Card>
