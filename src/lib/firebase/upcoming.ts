...
export const getUpcomingSlots = async (): Promise<
	Result<UpcomingSlot[], Error>
> => {
	try {
...
export const initializeUpcomingSlots = async (): Promise<
	Result<void, Error>
> => {
...
export const initializeNextWeekSlots = async (
	channelIds: string[],
): Promise<void> => {
...
export const deleteUpcomingSlot = async (
	channelId: string,
): Promise<Result<void, Error>> => {
	try {
		const channelsRef = collection(db, "upcoming");
