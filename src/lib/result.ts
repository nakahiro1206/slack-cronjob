import { z, ZodAny, ZodInvalidReturnTypeIssue, ZodTypeAny } from "zod";
...
export const safeFetch = async <S extends ZodTypeAny>(
    ...
    try {
        ...
        if (!response.ok) {
            return Err<z.infer<S>, Error>(new Error(`HTTP error! status: ${response.status}`));
        }
        const json = await response.json();
        const parsed = schema.safeParse(json);
        if (!parsed.success) {
            return Err<z.infer<S>, Error>(new Error(`Invalid response: ${JSON.stringify(parsed.error)}`));
        }
        return Ok(parsed.data);
    } catch (error) {
        return Err<z.infer<S>, Error>(new Error(`Failed to fetch ${target}: ${error}`));
    }
}