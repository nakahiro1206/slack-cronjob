import { ChannelDatabaseRepositoryInterface } from "./interfaces";
...
export class ChannelService {
    ...
    addChannel = async (input: Channel): Promise<{ success: boolean,  error?: string}> => {
    ...
    updateChannel = async (input: Pick<Channel, "channelId" | "channelName" | "day">): Promise<{ success: boolean, error?: string }> => {
        const result = await this.channelDatabaseRepository.updateChannel(input.channelId, {
    ...
    deleteChannel = async (channelId: string): Promise<{ success: boolean; error?: string }> => {
        const result = await this.channelDatabaseRepository.deleteChannel(channelId);
        return result.match<{ success: boolean }>(
            () => {
                return { success: true };
            },
